 ...
 */
export function getExchangeRate(): Promise<IDatExchg> {
  const prom = new Promise<IDatExchg>((resolve, reject) => {
    urlGet(url)
      .then((xlmString: string) => {
        ...
        const iErg: IDatExchg = {
          datumAsIso: (moment(datum) as Moment).toISOString(),
          exchange_rate: rate
        }
 ...
 */
export function getExchangeRateLast90Days(): Promise<IDatExchg[]> {
  const prom = new Promise<IDatExchg[]>((resolve, reject) => {
    urlGet(url90)
      .then((xlmString: string) => {
        const doc = new dom().parseFromString(xlmString)
        ...
        const listCube: Element[] = sele2("//xmlns:Cube[@time]", doc)

        const exchTabelle: IDatExchg[] = []

        const test2 = listCube.map(ele => {
          // get Datum...
          // Achtung: um mit XPATH im angegebenen Context zu suchen (=> ele) muss ".//" genommen werden statt "//"
          const neuDatExchg: IDatExchg = { datumAsIso: "", exchange_rate: 0 }
